...
rule mapping:
    ...
    log:
        "logs/bwa_{smp}.log",
    message:
        """--- Mapping {wildcards.smp} to H37Rv ---"""
    shell:
        """
...
rule compute_coverage:
    ...
    output:
        bed="BED/{smp}.bed",
    message:
        """--- Computing coverage of {wildcards.smp}.bam ---"""
    shell:
        """
...
rule make_vcf:
    ...
    output:
        vcf="VCF/{smp}_raw.vcf",
    message:
        """--- Making {wildcards.smp}_raw.vcf ---"""
    shell:
        """
...
rule clean_vcf:
    input:
        "VCF/{smp}_raw.vcf",
    output:
        "VCF/{smp}_cleaned.vcf",
    message:
        """--- Cleaning {wildcards.smp}_raw.vcf ---"""
    run:
...
rule change_samplenames:
    ...
    output:
        "VCF/{smp}_final.vcf",
    log:
        "logs/gatk_addnames_{smp}.log",
    message:
        """--- Adding samplename to {wildcards.smp}_cleaned.vcf ---"""
    shell:
        """
...
rule compress_vcf:
    input:
        "VCF/{smp}_final.vcf",
    output:
        "VCF/{smp}_final.vcf.gz",
    log:
        "logs/bcftools_comp_{smp}.log",
    message:
        """--- Compressing {wildcards.smp}.vcf ---"""
    shell:
        """
...
rule index_vcf:
    input:
        "VCF/{smp}_final.vcf.gz",
    output:
        "VCF/{smp}_final.vcf.gz.csi",
    log:
        "logs/bcftools_index_{smp}.log",
    message:
        """--- Indexing VCF ---"""
    shell:
        """
...
rule estimate_depth:
    input:
        vcf="VCF/{smp}_final.vcf.gz",
        csi="VCF/{smp}_final.vcf.gz.csi",
        fasta="db/H37Rv.fna",
        bam="BAM/{smp}.bam",
        bai="BAM/{smp}.bam.bai",
    output:
        "VCF/{smp}_final.duphold.vcf",
    log:
        "logs/duphold_{smp}.log",
    conda:
        "envs/duphold.yaml"
    message:
        """--- Estimating depth of flanking regions in {wildcards.smp}.bam ---"""
    shell:
        """
...
rule convert2table:
    ...
    output:
        "RD_table_raw.tsv",
    message:
        """--- Converting VCF to tab-separated table ---"""
    log:
        "logs/gatk_vcf2table.log",
    shell:
        """
...
rule clean_table:
    ...
    run:
        ...
        df.rename(
            columns={"POS": "START", "SVLEN": "LENGTH", "SVTYPE": "TYPE"}, inplace=True
        )
...
rule annotate_RD:
    ...
    run:
        ...
        e2 = df2.END.values
        r = (
            pd.DataFrame(((e2 - s1[:, None]) > 0) & ((s2 - e1[:, None]) < 0))
            .dot(df2.RD + "|")
            .str[:-1]
        )
        df1["RD"] = r.values
...
rule compute_RD_coverage:
    input:
        ...
        rd="db/RD.bed",
    output:
        glo="BED/{smp}.mosdepth.global.dist.txt",
    ...
    params:
        prefix="BED/{smp}",
    message:
        """--- Computing deleted regions coverage in {wildcards.smp}.bam ---"""
    shell:
        """
...
rule decompress:
    input:
        bedgz="BED/{smp}.regions.bed.gz",
    output:
        bed="BED/{smp}.regions.bed",
    threads: 2
    message:
        """--- Decompressing {wildcards.smp}.regions.bed.gz ---"""
    shell:
        """
...
rule calculate_proportion:
    input:
        "BED/{smp}.regions.bed",
    output:
        "BED/{smp}.proportion.bed",
    message:
        """--- Calculating read depth ratio of RD regions in {wildcards.smp} ---"""
    run:
        df = pd.read_csv(
            input[0],
            sep="\t",
...
rule concatenate:
    input:
        expand("BED/{smp}.proportion.bed", smp=SAMPLES),
    output:
        "all_concat.bed",
    message:
...
rule make_tables:
    ...
    log:
        "logs/r_tables.log",
    shell:
        "Rscript scripts/makeTables.R -T {config[threshold]} -i {input} -x {output.xlsx} -t {output.tab} -b {output.bin} 2> {log}"
